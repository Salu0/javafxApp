...
class EscapeProcessor {
    ...
    private static String processConvertToken(String functionToken, boolean serverSupportsConvertFn, MySQLConnection conn) throws SQLException {
        ...
        if (serverSupportsConvertFn) {
            ...
            // CAST/CONVERT, so we can't re-read some data type conversions (date,time,timestamp, datetime)

            if (newType == null) {
                throw SQLError.createSQLException(
                        "Can't find conversion re-read for type '" + type + "' that is applicable for this server version while processing escape tokens.",
